...
class EspHal : public RadioLibHal {
  public:
    ...
    void yield() override {
      // Yield to other tasks during long RadioLib operations (e.g., transmit/receive)
      // This allows the idle task to run and reset the watchdog timer
      taskYIELD();
    }

    unsigned long millis() override { 
      // Use FreeRTOS ticks for much faster performance (avoids expensive timer calls and 64-bit division)
      // portTICK_PERIOD_MS is typically 10ms on ESP32, so this gives ~10ms resolution which is fine for RadioLib
